...
class LMG_Loss(torch.nn.Module):
    def __init__(self,patch_size):
        super(LMG_Loss,self).__init__()
        self.kernel = torch.FloatTensor([[0, -1, 0],[-1, 0, 1],[0, 1, 0]]).expand(1,1,3,3)
        self.weight = torch.nn.Parameter(data=self.kernel,requires_grad=False).cuda()
        self.patch_size = patch_size


    def forward(self, pred):
        channels = pred.shape[1]
        gradient_max_channels = torch.zeros_like(pred)
        for i in range(channels):
            gradient_max_channels[:,i,:,:] = F.max_pool2d(
                torch.abs(torch.conv2d(pred[:,i,:,:].unsqueeze(1),self.weight,bias=None,stride=1,padding=1)),
                self.patch_size,
                stride=1,
                padding=int((self.patch_size-1)/2)).squeeze(1)

        gradient_max_channels = torch.max(gradient_max_channels, dim=1).values
        return F.l1_loss(gradient_max_channels,torch.ones_like(gradient_max_channels)*2)
